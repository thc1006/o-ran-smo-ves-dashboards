...
Published to the community catalogue — import by ID inside your Grafana:

| Dashboard | Community ID | Direct URL |
|---|---|---|
| VES NR cell DU measurement | `25190` | <https://grafana.com/grafana/dashboards/25190> |
| VES NR cell CU measurement | `25189` | <https://grafana.com/grafana/dashboards/25189> |

The portable exports (with `${DS_INFLUXDB}` datasource variable and
`__inputs` block) live in `dist/grafana-com/` — generated by
`scripts/prepare-for-grafana-com.py`. Forks that ship new dashboards
should re-run that script before uploading.

## Screenshots & regeneration

[`scripts/capture-screenshots.py`](scripts/capture-screenshots.py)
drives a headless Chromium via Playwright: brings up the demo stack,
logs into Grafana, hides nav chrome via CSS, waits for panels to render,
and writes PNGs under `docs/screenshots/`. Run it after any dashboard
JSON change before committing new previews.
...
All screenshots referenced from this README live under
`docs/screenshots/`.
